...
function lay_out_cross_section_nodes(L, θ)
    ...
    cross_section = Array{Matrix{Float64}}(undef, num_segments)
    ...
    for i in eachindex(L)
        ...
        if i == 1
            ...
            start_node = vec(cross_section[i-1][end, :])

        end

        cross_section[i] = Geometry.transform_line_segment_to_global(L[i], start_node, θ[i])
...
function generate_cross_section_rounded_corners(cross_section_nodes, r, n)
   ...
   for i in eachindex(r)
        ...
        A = vec(cross_section_nodes[i]) #need vector format here to fillet calculations
        B = vec(cross_section_nodes[i+1])
        C = vec(cross_section_nodes[i+2])
    
        corners[i] = Geometry.generate_fillet(A, B, C, r[i], n[i])
...
function generate_straight_line_segments(cross_section_nodes, corners, n)
    ...
    for i in eachindex(n)
        ...
        segments[i] = Geometry.discretize_segment(A, B, n[i])
...
function generate_open_cross_section(L, θ, n, n_radius)

    cross_section_nodes = lay_out_cross_section_nodes(L, θ)

    corners = generate_cross_section_rounded_corners(cross_section_nodes, r, n_radius)
